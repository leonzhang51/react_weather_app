/**
 * Énoncé du second travail pratique (Cours 582P51MA_gr17600)
 * Web APP basé sur openweather API
...
$("#saveSetting").click(function () {
    try {
        ...
        //obtenir le id de la ville
        console.log($("#villePrefer").val());
        if ($("#villePrefer").val() !== "Choisir la ville") {
...
function showWeather(lat, lon, celsius) {
    $.getJSON(OW_API.get_forecast_url(lat, lon), function (data, status) {
        ...
        if (celsius) {
            $(".card .card-body .temperature span").html(CONV.k_a_c(data.list[0].main.temp) + " &#8451;"); //afficher dans DEGREE CELSIUS
        } else {
            $(".card .card-body .temperature span").html(data.list[0].main.temp + " &#8457;"); //afficher dans DEGREE FAHRENHEIT
        }
        ...
        for (let i = 0; i < 24; i++) {
            ...
            if (celsius) {
                $("#" + i + " .temperature span").html(CONV.k_a_c(data.list[i].main.temp) + " &#8451;"); //afficher dans DEGREE CELSIUS
            } else {
                $("#" + i + " .temperature span").html(data.list[i].main.temp + " &#8457;"); //afficher dans DEGREE FAHRENHEIT
            }
